...
const customTools = [
  ...
  },
  {
    ...
    function: {
      name: 'data_query',
      description: 'Query structured research data or internal databases.',
      parameters: {
        type: 'object',
        properties: {
          query: { type: 'string', description: 'The query to execute against the research data.' },
          type: { type: 'string', description: "Type of data to query (e.g., 'papers', 'reports', 'market_trends')." }
        },
        required: ['query', 'type']
      }
    }
      ...
      case 'data_query':
        return {
            content: JSON.stringify({
                results: [
                    { title: 'AI in Cloudflare Workers', author: 'Cloudflare Team', year: 2024, summary: 'Overview of AI inference on the edge.' },
                    { title: 'Durable Objects for State Management', author: 'Cloudflare Developers', year: 2023, summary: 'Scalable stateful applications with Durable Objects.' }
                ],
                query: args.query,
                type: args.type
            })
